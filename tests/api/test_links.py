...
class TestLinkStats:
    async def test_link_stats_success(self, client: AsyncClient):
        ...
        response = await client.get(f"/links/{code}/stats")
    ...
    async def test_link_stats_not_found(self, client: AsyncClient):
        ...
        response = await client.get("/links/doesnotexist/stats")
    ...
    async def test_multiple_links_stats(self, client: AsyncClient):
        ...
        response1 = await client.post(
            "/links", json={"original_url": "https://example.com/1"}
        )
        code1 = response1.json()["code"]

        response2 = await client.post(
            "/links", json={"original_url": "https://example.com/2"}
        )
        code2 = response2.json()["code"]

        stats_response = await client.get(f"/links?codes={code1}&codes={code2}")
    ...
    async def test_multiple_links_stats_with_invalid_code(self, client: AsyncClient):
        ...
        await client.post("/links", json={"original_url": "https://example.com/2"})

        stats_response = await client.get(f"/links?codes={code1}&codes=invalid")
        assert stats_response.status_code == 200
        data = stats_response.json()
        assert len(data) == 1
...
class TestRedirect:
    ...
    async def test_redirect_increments_click_count(self, client: AsyncClient):
        ...
        stats_response = await client.get(f"/links/{code}/stats")
...
class TestDeleteLink:
    ...
    async def test_delete_link_not_found(self, client: AsyncClient):
        ...
        response = await client.delete("/links/doesnotexist")
        assert response.status_code == 404
    ...
    async def test_delete_link_then_get_stats(self, client: AsyncClient):
        ...
        await client.delete(f"/links/{code}")
...
class TestCleanupExpired:
    async def test_cleanup_deletes_only_expired_links(self, client: AsyncClient):
        ...
        stats = await client.get(f"/links/{active_code}/stats")
