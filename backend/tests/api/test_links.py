...
class TestCreateLink:
    async def test_create_link_success(self, client: AsyncClient):
        ...
        response = await client.post(
            "/api/links", json={"original_url": "https://example.com"}
        )
    ...
    async def test_create_link_invalid_url(self, client: AsyncClient):
        ...
        response = await client.post(
            "/api/links", json={"original_url": "not-a-valid-url"}
        )
...
class TestLinkStats:
    ...
    async def test_link_stats_not_found(self, client: AsyncClient):
        ...
        response = await client.get("/api/links/doesnotexist/stats")
    ...
    async def test_multiple_links_stats(self, client: AsyncClient):
        ...
        response1 = await client.post(
            "/api/links", json={"original_url": "https://example.com/1"}
        )
        code1 = response1.json()["code"]

        response2 = await client.post(
            "/api/links", json={"original_url": "https://example.com/2"}
        )
        code2 = response2.json()["code"]

        stats_response = await client.get(f"/api/links?codes={code1}&codes={code2}")
    ...
    async def test_multiple_links_stats_with_invalid_code(self, client: AsyncClient):
        ...
        await client.post("/api/links", json={"original_url": "https://example.com/2"})

        stats_response = await client.get(f"/api/links?codes={code1}&codes=invalid")
        assert stats_response.status_code == 200
        data = stats_response.json()
        assert len(data) == 1
...
class TestRedirect:
    ...
    async def test_redirect_not_found(self, client: AsyncClient):
        ...
        response = await client.get("/r/doesnotexist", follow_redirects=False)
        assert response.status_code == 404
    ...
    async def test_redirect_increments_click_count(self, client: AsyncClient):
        ...
        stats_response = await client.get(f"/api/links/{code}/stats")
...
class TestDeleteLink:
    ...
    async def test_delete_link_not_found(self, client: AsyncClient):
        ...
        response = await client.delete("/api/links/doesnotexist")
        assert response.status_code == 404
    ...
    async def test_delete_link_then_get_stats(self, client: AsyncClient):
        ...
        await client.delete(f"/api/links/{code}")
...
class TestCleanupExpired:
    async def test_cleanup_deletes_only_expired_links(self, client: AsyncClient):
        ...
        response = await client.delete("/api/links/expired")
        assert response.status_code == 200
        assert response.json() == {"deleted": 1}

        stats = await client.get(f"/api/links/{active_code}/stats")
...
class TestBackendEntrypoints:
    async def test_root_is_not_served_by_backend(self, client: AsyncClient):
